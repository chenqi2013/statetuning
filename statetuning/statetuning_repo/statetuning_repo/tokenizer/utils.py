...
class PIPELINE():
    ...
    def generate(self, ctx, token_count=100, args=PIPELINE_ARGS(), callback=None, state=None, sign=None, fastw=None):
        all_tokens = []
        out_last = 0
        out_str = ''
        occurrence = {}
        for i in range(token_count):
            ...
            while len(tokens) > 0:
                out, state = self.model.forward(tokens[:args.chunk_len], state=state, sign=sign, fastw=fastw)
